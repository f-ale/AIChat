    ...
    suspend fun sendMessage(character: Character, conversation: Conversation, message: String) {
        ...
        try {
            val pastMessages = messageDao.getLatestMessagesForConversation(
                conversation.id,
                5 // TODO: Calculate number of tokens and retrieve the right amount of messages
            )
            ...
            db.withTransaction {
                val prompt = "You are ${character.name}. Do not write replies as User. ${character.attributes}\n" + pastMessages.joinToString(
                    separator = "\n",
                ) { it ->
        ...
        } catch (e: Exception) { // TODO: Use a different approach to error handling
